namespace Test
{
    ...
    public static class Program
    {
        ...
        {
            Console.WriteLine("ModelTokenizer Test Program");
            Console.WriteLine("==========================");
            ...
            try
            {
                ...
                if (!isConnected)
                {
                    Console.WriteLine("ERROR: Could not connect to the model tokenizer service.");
        ...
        private static async Task TestSingleTokenization(ModelTokenizer tokenizer)
        {
            Console.WriteLine("\n--- Single Text Tokenization Test ---");

            try
            {
                string text = "Hello world! This is a test sentence for the model tokenizer.";
                Console.WriteLine($"Text to tokenize: \"{text}\"");

                // Tokenize single text
                Console.WriteLine("Tokenizing...");
                var result = await tokenizer.Tokenize(text, "sentence-transformers/all-MiniLM-L6-v2");
        ...
        private static async Task TestBatchTokenization(ModelTokenizer tokenizer)
        {
